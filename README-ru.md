...
Для каждого BLOB-поля на источнике создаётся дополнительная таблица YDB следующей структуры:
```sql
...
Аналогичная таблица создаётся для каждого CLOB-поля:
```sql
...
CLOB-таблица создаётся для исходных колонок типа CLOB или NCLOB, а также для текстовых колонок, явно помеченных через `<clob-column>` в разделе `<table-ref>`.
...
Запись BLOB-таблицы хранит не более 64 Кбайт двоичных данных в поле `val`, запись CLOB-таблицы хранит не более 32768 символов. Последовательность блоков данных определяется значением поля `pos`.
...
<ydb-importer>
    ...
    <source type="generic|postgresql|greenplum|mysql|mariadb|oracle|mssql|db2|informix|clickhouse|hana|vertica">
        ...
        <!-- Количество повторных попыток чтения партиции при ошибке источника,
             ...
             Применяется к партиционированным таблицам без BLOB- и CLOB-колонок.
         -->
