...
public class LoadDataTask implements Callable<Boolean> {
    ...
    /** Row batching. Regroups rows into one buffer per YDB partition when bounds
     *  are known, otherwise fills a single buffer in read order. */
    ...
    /** Arrow columnar batching. Regroups rows into one batch per YDB partition when
     *  bounds are known, otherwise fills a single batch in read order. */
